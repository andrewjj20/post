...
impl FindService{
    pub fn new(_handle: tokio::runtime::Handle) -> FindService {
        ...
        let _proc = _handle.enter(|| {
            ...
            let ret = tokio::spawn(tokio::process::Command::new(path)
                .arg("--bind")
                .arg(bind)
                .kill_on_drop(true)
                .spawn().expect("Failed to start meetup"));
    ...
    pub async fn server_status(&self) -> std::result::Result<proto::StatusResponse, Box<dyn Error>> {
